...
async function getNewRepos(
  ...
  const repos = [];
  for await (const repo of loadRepos(criteria)) {
    repos.push(repo);
  }
  ...
  // Get new repos and old repos that have increased to the next `threshold` multiple of stars
  const newRepos = new Set(
    db.query(
      ...
      SELECT new_repos.name, new_repos.stars FROM new_repos LEFT JOIN seen_repos USING(name) WHERE seen_repos.stars IS NULL OR new_repos.stars - new_repos.stars % ? > seen_repos.stars
      ON CONFLICT(name) DO UPDATE SET stars=excluded.stars
      RETURNING name, stars`,
      [...repos.flatMap((repo) => [repo.name, repo.stars]), threshold],
